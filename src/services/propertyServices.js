export const getProperties = async (category_id) => {
    ...
    try {
        const response = await fetch(process.env.production.REACT_APP_API_ENDPOINT + 'properties', {
            headers: {
                'Authorization': token
            }
        })
...
export const searchProperties = async (keyword) => {
    ...
    try {
        const response = await fetch(process.env.production.REACT_APP_API_ENDPOINT + `properties?keyword=${keyword}`, {
            headers: {
                'Authorization': token
            }
        })
